...
export function createHyperSubLayer(
  ...
  hyperPlusKeyAloneMap?: { [key_code in KeyCode]?: string }
): Manipulator[] {
  ...
  return [
    // When Hyper + sublayer_key is pressed, set the variable to 1; on key_up, set it to 0 again
    {
      ...
      to_if_alone:
        hyperPlusKeyAloneMap && hyperPlusKeyAloneMap[sublayer_key]
          ? [
              {
                shell_command: hyperPlusKeyAloneMap[sublayer_key],
              },
            ]
          : undefined,
      to: [
...
export function generateTopLevelHyperKey(
  key: KeyCode,
  appName: string
): Manipulator {
  return {
...
export function openLinkInChrome(args: string) {
  return {
    to: [
      {
      ...
      {
        shell_command: `~/Dropbox/Dev/Raycast/Scripts/open-link-in-chrome.sh ${args}`,
      },
    ],
